...
static void ogg_get_current_sample(sigrenderer_t* renderer, float volume, sample_t* sample);
...
DUH *dumb_load_ogg(const char *filename, int loop)
{
	...
	((OGG_SIGNAL*)signal)->filename = (char*)malloc(sizeof(*((OGG_SIGNAL*)signal)->filename) * (strlen(filename) + 1));
...

	{
		...
		((OGG_SIGNAL*)signal)->length = (long)ov_pcm_total(&ov, -1);
	...
	return make_duh(((OGG_SIGNAL*)signal)->length, 0, NULL, 1, &descptr, &signal);
}
