...
namespace Vm
{
	...
	void VirtualMachineSingleton::ExecuteBytecode(const Method& method, Util::Stack<Variant*>& argumentStack)
	{
		...
		while(ip < size)
		{
			...
			switch(instruction)
			{
				...
				case Instruction::GetIndex:
				{
					...
					switch(value->TypeOf())
					{
						case Variant::Type::String:
						{
							...
							charValue += value->StringAt(idx);
							stack.Push(CreateVariant(charValue));
							break;
						}

						case Variant::Type::List:
							stack.Push(CreateVariant(*value->ListAt(idx)));
							break;
				...
				case Instruction::GetThis:
					stack.Push(CreateVariant(Variant(method.parent, false)));
					break;
				...
				case Instruction::IncrementVariable:
					variables[instruction[0]] = CreateVariant(*variables[instruction[0]] + instruction[1]);
					break;
				...
				{
					// Get class pointer
					Variant* parentVariant = stack.Pop();
					Class* parent = parentVariant->Object();
					if (parent == nullptr)
						throw Exception("Cannot call instance method of null instance.");

					// Resolve method
					std::string sMethod = instruction[0];
					Method cMethod = parent->GetMethod(sMethod, static_cast<size_t>(instruction[1].Integer()));
					if (cMethod.Type() == Method::Type::Illegal)
						throw Exception("Instance class method '" + sMethod + "' doesn't exist.");

					// Prepare debugging information
					debugMethodStack.Push(DebugScope(parent->Name(), cMethod.Name(), ip));

					// Execute method
					Util::Stack<Variant*> argStack(stack, cMethod.ArgumentCount());
					cMethod.Execute(argStack, parent);

					// HACK Like I give a /single/ fuck
					if (cMethod.Name() == VM_CLASS_CTOR_NAME)
						argStack.Push(parentVariant);

					stack.Push(argStack.Pop());

					// This is kind of a hack, whatever
					if (cMethod.Type() == Method::Type::Hooked)
						debugMethodStack.Pop();
				...
				case Instruction::New:
					stack.Push(CreateVariant(Variant(new Class(classes[instruction.Operand()]), true)));
					break;
	...
	void VirtualMachineSingleton::Execute(const Method& method, Util::Stack<Variant*>& argumentStack)
	{
		...
		catch(const Variant::Exception& e)
		{
			// Variant Exception
			std::stringstream ess;
	...
	void VirtualMachineSingleton::CollectGarbage(Util::Stack<Variant*>& stack)
	{
		...
		// Print results
		/*VerboseTrace << "GC Collected "
					 << static_cast<int>(initialObjectCount - gcObjectCount)
					 << " objects, "
					 << gcObjectCount
					 << " remaining."
					 << lendl;*/
