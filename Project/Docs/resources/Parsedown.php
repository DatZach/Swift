...
class Parsedown
{
                ...
                case 'table':
                    ...
                    if ($line[0] === '|')
                    {
                        ...
                        foreach ($parts as $index => $part)
                        {
							...
							// Swift Docs additions
							$substring = str_replace('$', '<img src="resources/images/icons/static.png" alt="Static">', $substring);
							$substring = str_replace('%g', '<img src="resources/images/icons/get.png" alt="Get">', $substring);
							$substring = str_replace('%s', '<img src="resources/images/icons/set.png" alt="Set">', $substring);
                    ...
                    if ($line[0] === '|' and $block['content'][0] === '|' and chop($line, ' -:|') === '')
                    {
                        ...
                        foreach ($parts as $index => $part)
                        {
                            ...
                            $nestedBlock = array(
                                'name' => 'th',
                                'content type' => 'line',
                                'content' => $substring // SWIFT DOCS ADDITION -----------------> $this->parseLine($substring),
                            );
            ...
            switch ($line[0])
            {
                case '#':
                    ...
                    if (isset($line[1]))
                    {
						...
						// Swift Docs additions
						$string = str_replace('$', '<img src="resources/images/icons/static.png" alt="Static">', $string);
						
						if ($level === 2)
						{
							$string = str_replace('class', '<span class="h2-keyword">class </span>', $string);
							$string = str_replace('method', '<span class="h2-keyword">method </span>', $string);
							$string = str_replace('property', '<span class="h2-keyword">property </span>', $string);
							$string = str_replace('field', '<span class="h2-keyword">field </span>', $string);
						}
						
						// TODO Fade method and class keywords in h2s
                ...
                case '[':
                    ...
                    if (isset($element))
                    {
                        ...
                        else
                        {
							...
							// Swift Docs Modification
							$processedLink = $element['link'];
							if (substr($processedLink, 0, 1) === '/')
								$processedLink = self::$DOCS_ROOT . substr($processedLink, 1, strlen($processedLink) - 1) . '.md';
								
                            $markup .= '<a href="'.$processedLink.'"';
	...
	private static $DOCS_ROOT = 'http://www.x2048.com/swift/docs/index.php?path=';
}
