...
namespace Vm
{
		  ...
		  next(nullptr)
	{
		switch(type)
		{
			...
			case Type::EngineObject:
				engineObjectValue = other.engineObjectValue;
				break;

			default:
				throw Exception("An internal error occurred when copying a variant.");
		}
	}
	...
	Variant::Variant(const std::vector<Variant*>& value)
		: type(Type::List),
		  listValue(new std::vector<Variant*>(value)),
		  next(nullptr),
		  marked(false)
	{
	...
	Variant::Variant(Class* value)
		: type(Type::Object),
		  objectValue(std::shared_ptr<Class>(value,
			[](Class* parent)
			{
				// Call destructor if one exists
				Method& method = parent->Method(VM_CLASS_DTOR_NAME, 0);
				if (method.Type() != Method::Type::Illegal)
					method.Execute(Util::Stack<Variant*>(), parent);
	...
	Variant::Variant(void* value, void (*del)(void*))
		: type(Type::EngineObject),
		  engineObjectValue(std::shared_ptr<void>(value, del, std::allocator<void>()))
	{
	...
	Variant::operator std::basic_string<char>&() const
	{
		return String();
	}
