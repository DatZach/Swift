...
namespace Vm
{
	...
	private:
		...
		std::shared_ptr<Class>					objectValue;
		std::shared_ptr<void>					engineObjectValue;
	...
	public:
		...
		Class* Object() const;
		template <class T>
		T* EngineObject() const { return static_cast<T*>(engineObjectValue.get()); }
	...
	inline Variant operator +(const Variant& left, const Variant& right)
	{
		switch(left.TypeOf())
		{
			...
			{
				// NOTE Can probably fix that bug rewriting this somehow
				// ALSO NOTE new Variant() creates untracked objects and should NOT be done
